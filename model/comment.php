...
{
    $sql = "INSERT INTO `duan`.`binh_luan`(`ma_khach_hang`, `ngay_binh_luan`, `noi_dung`, `ma_san_pham`, `rate`)" . "VALUES($ma_khach_hang, '$ngay_binh_luan', '$noi_dung', $ma_san_pham, $rate);";
    pdo_execute($sql);
}
...
{
    $sql = "select * from binh_luan" . "INNER JOIN khach_hang ON binh_luan.ma_khach_hang = khach_hang.ma_khach_hang" . "where ma_san_pham= $ma_san_pham order by ma_binh_luan desc";
    $list_cmt = pdo_query($sql);
    return $list_cmt;
}
